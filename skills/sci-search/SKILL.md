...
критичных случаев предпочитать `get_page_text` через браузер (§5 ниже) — он отдаёт
...
недоступен, а нужен настоящий браузер: встроенный Browser pane
(`mcp__Claude_Browser__*`, запрет снят оператором 13.09, доктрина **#WEB-1**) либо
`mcp__claude-in-chrome__*` (Chrome оператора с его залогиненной сессией). Домен
заблокирован политикой харнесса — сразу Chrome, в панели обхода нет. Браузер проходит
челлендж самим фактом того, что является браузером; свою вкладку после чтения
закрыть (`tabs_close` / `tabs_close_mcp`). CAPTCHA и
