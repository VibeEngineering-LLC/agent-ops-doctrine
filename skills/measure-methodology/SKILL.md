...
контурами, не видевшими друг друга: методика производительности прошивок (контур прошивок ESP32,
`perf-methodology.md`: «вся история багов должна стать методикой») и база ловушек бенчмарка
локальных моделей (аудит-кода, 11 дефектов стенда до первой публикации). Убрав предмет, оба
...
- Не субъективную проверку человеком заменять автотестом: ручная навигация поймала лаги,
  которых автотесты не видели (контур прошивок ESP32, P-023); чтение сырого ответа поймало дефект, которого не
  видел аудит (аудит-кода, дефект 7). Это не слабость метода, а отдельный его канал.
...
| Прошивки ESP-IDF | скилл `firmware-stability` + `<контур прошивок>\references\perf-methodology.md` (зона контура прошивок, читать) | наблюдаемость до нагрузки, метрики RAM/RSSI/ping, регрессионный гейт RAM, протокол энергопотребления |
...
прошивки «заполняется под каждую плату»).
...
- 2026-08-23 — создан контуром аудит-кода по решению оператора после того, как контур прошивок ESP32
  (`perf-methodology.md`) и контур аудита кода (`llm-benchmark-pitfalls.md`) независимо написали
  документы одного класса. Ядро — общее обоих; профиль LLM — из базы ловушек; профиль
  прошивок остаётся в зоне контура прошивок ESP32 (§12), сюда — только ссылка.
