...
**Установлено 2026-06-19 по запросу оператора, общий для всех контуров:** MCP-сервер `markitdown` (user-scope в `<home>\.claude.json`, инструмент `convert_to_markdown(uri)` — `http:`/`https:`/`file:`/`data:`) + CLI `C:\Users\<you>\.local\bin\markitdown.exe`. Доступен любой сессии после её старта (`claude mcp list` → `markitdown ✓ Connected`).
...
Обратное (документ → md) — НЕ этот инструментарий: `markitdown` (раздел 1), `doc-extract`, приватный RAG-скилл, `drawings-spec`, `unlimited-ocr` (раздел 3). Дисциплина: SSOT `.md`/`.py`; регенерация одной командой; явный путь (§18) к исходнику И артефакту; bulk-шаблоны → Ollama данные → Python пишет; юр-тексты — Claude черновит, НЕ публикует (gate оператора + юрист РФ). Полные таблицы/pandoc-команды: `<home>\.claude\references\office-docs-toolkit.md`.
...
| Семантический поиск по личным документам | приватный RAG-скилл (не публикуется) |
