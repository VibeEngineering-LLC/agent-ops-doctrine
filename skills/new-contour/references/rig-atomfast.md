...
> замена — файловый ящик `_interchat\inbox\`, #BUS-2). Пошаговая отчётность надзорному контуру
> через `step_report.py` ОТМЕНЕНА вместе с сужением его роли (оператор, 2026-08-30):
> надзорный не часть рабочего процесса и работу контуров не контролирует. Контур
...
> в `~/lab/logs/`. По done — отправь надзорному контуру step_report с: dumpsys battery
...
файлы в private-папку этого же test-APK, читает системные dump'ы). надзорный
