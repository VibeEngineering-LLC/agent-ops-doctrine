...
| Проект | Папка |
|---|---|
| приватный контур (шаблоны, smoke-tests, helpers, Ollama-обработка) | `<project>\scripts\<назначение>\` |
...
## #BASH-1 — перенесено из ядра (DOC-1 шаг 2, 2026-09-13)

**#BASH-1 (надзорный, W-041, 2026-08-23): Windows-путь к папке в двойных кавычках Bash,
заканчивающийся на `\`, ломает парсинг.** POSIX-shell внутри `"..."` читает `\"` как
экранированную кавычку, не как конец строки — `ls -la "C:\...\имя\"` уходит в `unexpected EOF`
(команда не выполняется вообще). ИМПЕРАТИВ: Windows-пути в Bash-командах — **forward slash**
(`C:/Users/<you>/...`, всегда безопасно в Git Bash) ЛИБО без конечного `\` перед закрывающей
кавычкой ЛИБО в одинарных кавычках (`\` внутри них буквален, trailing `\'` безопасен).

## #WRITE-1 — перенесено из ядра (DOC-1 шаг 2, 2026-09-13)

**#WRITE-1 (2026-08-28, надзорный, оператор: «не ошибайся больше!! запомни и создай правило»):
всё изложенное выше — ПРО СКРИПТЫ. Не-код (`.md`, `.json`, `.yaml`, `.txt`: письма, отчёты,
регламенты, доктрина) пишется ТОЛЬКО `Write`/`Edit` tool — всегда, независимо от размера.**
Запрещено: `bash → python - <<EOF → io.open().write()` и `cat > file.md <<EOF` для таких файлов.
Причина, стоившая четырёх провалов подряд в одной сессии: путь Windows проходит ТРИ парсера
(bash → heredoc → строка Python), и каждый по-своему читает `\`. `\N`, `\U`, `\a` — валидные
escape-последовательности: `\a` съедается молча (`D:\data\ai-library` записалось как
`D:\datai-library`), `\N`/`\U` роняют с `SyntaxError`, апострофы и backtick рвут heredoc
(`unexpected EOF`). Корень рецидива: §16 читался как общее правило записи файлов, хотя он про
код; на .md он не распространяется. Ту же ошибку в PowerShell-командах лечит `mcp__pwsh` (§27),
а не ручное экранирование в Bash.